...
function initGuideConfig(app) {
  if (app) {
    if (app.interview_guide_language) {
      selectedLanguage.value = app.interview_guide_language
    }
    if (app.interview_guide_preferences?.selected_sections) {
      selectedSections.value = app.interview_guide_preferences.selected_sections
    }
    if (app.interview_guide_preferences?.recursion_limit) {
      recursionLimit.value = app.interview_guide_preferences.recursion_limit
    }
    showConfigPanel.value = !app.interview_guide_html
  }
}
...
function handleCopyGuide() {
  if (!appStore.selectedApplication?.interview_guide_html) return
  const tempDiv = document.createElement('div')
  tempDiv.innerHTML = appStore.selectedApplication.interview_guide_html
      ...
      <div class="drawer-panel">
        ...
        <div v-else-if="appStore.selectedApplication" class="drawer-content">
          ...
          <div class="drawer-body">
            ...
            <div v-if="activeTab === 'guide'" class="guide-tab-panel animate-fade-in print-guide-panel">
              <!-- GENERATION RUNNING STATE -->
              <div v-if="isGeneratingGuide" class="state-container generating-state">
                ...
                <div class="config-card">
                  ...
                  <div class="config-grid">
                    <div class="input-group">
                      ...
                      <select v-model="selectedLanguage" class="form-input form-input-sm">
                        <option v-for="lang in LANGUAGES" :key="lang.code" :value="lang.code">
                          {{ lang.label }}
                        </option>
                      </select>
                      <span class="input-hint">AI generates natural phrasing in target language.</span>
                    </div>
                  </div>
                  ...
                  <div class="sections-picker-group">
                    <div class="sections-picker-header">
                      <label class="input-label">Select Modules ({{ selectedSections.length }}/{{ ALL_SECTIONS.length }})</label>
              ...
              <div v-else-if="appStore.selectedApplication?.interview_guide_html" class="guide-reader-layout animate-fade-in">
                <!-- Reader Action Toolbar -->
                <div class="reader-toolbar">
                  <div class="toolbar-left">
                    <span class="badge-lang">
                      <Globe :size="12" />
                      <span>{{ formatLanguageName(appStore.selectedApplication.interview_guide_language) }}</span>
                    </span>
                    <span v-if="appStore.selectedApplication.interview_guide_generated_at" class="timestamp-label">
                  ...
                  <div class="toolbar-right">
                    ...
                    >
                      <RotateCcw :size="14" />
                      <span class="hidden-xs">Re-Configure</span>
                    </button>
...
/* Guide Reader & Config Styles ported from Modal */
.state-container {
  display: flex;
  flex-direction: column;
  align-items: center;
