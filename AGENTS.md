...
- **Key Services:**
  ...
  - `llm_judge.py`: Automated zero-hallucination auditor validating generated deliverables (cover letters, application Q&A, interview guides) against verified candidate CV facts with configurable auto-rewrite recursion and telemetry diagnostic tracing.
...
- **System Settings & Onboarding:** `SystemSettingsModel` tracks global runtime settings, feature flags (`enable_email_intake`, `enable_embeddings`, `enable_web_search`, `enable_auto_cover_letter, enable_llm_judge`), search engine configuration (`search_provider`, `searxng_url`), and onboarding completion state (`has_completed_onboarding`).
