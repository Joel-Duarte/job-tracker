...
- **Local Development:** Run `./dev.sh` (or `./dev.sh --reset` to wipe and restart). This starts `db` (PostgreSQL), `scraper` (Camofox), `backend` (FastAPI), and `frontend` (Vite dev server) in detached mode using `docker-compose.dev.yml`. Use `./dev.sh --refresh-mocks` while the stack is running to reset and reseed the development dataset without restarting containers.
...
- Treat `backend/alembic/versions/` as the source of truth for schema changes. Do not add new runtime `ALTER TABLE` or `CREATE TABLE` mutations to replace a migration.
- When changing a SQLAlchemy model, create and review a migration from `backend/`:
   ```bash
   uv run alembic revision --autogenerate -m "describe schema change"
   uv run alembic upgrade head
   ```
- Inspect autogenerated revisions manually before applying them. Confirm table names, column nullability, defaults, indexes, foreign keys, extensions, and upgrade/downgrade behavior; remove unrelated metadata churn.
- Never rewrite a migration that has already been applied to shared or production databases. Add a new revision instead. Only amend an unapplied revision when the branch has not exposed it to another database.
- Keep `backend/alembic/env.py` metadata imports synchronized with registered models so autogeneration sees the complete schema.
- Validate migration changes with `uv run pytest` from `backend/`. Migration tests must use an isolated database and must not rely on tables left by ORM metadata creation.
