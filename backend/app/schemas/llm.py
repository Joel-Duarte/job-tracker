...
class VocabularyTranslationItem(BaseModel):
    ...
    cv_term: str = Field(description="Equivalent terminology currently used in the candidate CV")
    replacement_guidance: str = Field(description="Specific instructions for swapping the term")


class ImpactReframingItem(BaseModel):
    bullet_point: str = Field(description="Original bullet point from the CV")
    suggested_rewrite: str = Field(description="Reframed bullet point aligning with JD action verbs and metrics")
    reason: str = Field(description="Rationale for how this reframe improves ATS rank / recruiter appeal")
...
class OptimizationGaps(BaseModel):
    ...
    experience_mismatch: Optional[str] = Field(
        default=None,
        description="Years of experience or seniority level delta if any"
    )
...
class JobAssessmentResult(BaseModel):
    ...
    hard_matches: Optional[HardMatches] = Field(
        default=None,
        description="Core hard keyword matches and top aligned skills"
    )
    optimization_gaps: Optional[OptimizationGaps] = Field(
        default=None,
        description="Strict terminology mismatches and experience delta"
    )
    tailoring_strategy: Optional[ResumeTailoringStrategy] = Field(
        default=None,
        description="Step-by-step resume tailoring recommendations"
    )
    markdown_report: Optional[str] = Field(
        default=None,
        description="Full structured Markdown audit report"
    )
