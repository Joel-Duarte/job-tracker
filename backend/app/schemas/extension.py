...
class ClipUrlRequest(BaseModel):
    ...
    raw_html: Optional[str] = Field(default=None, description="Optional pre-captured HTML from DOM")


class ClipJobRequest(BaseModel):
    company: str = Field(..., min_length=1, description="Company name captured from page")
    ...
    description: Optional[str] = Field(default="", description="Captured job description text")
    external_job_id: Optional[str] = Field(default=None, description="Requisition or Job ID if found")
    status: Optional[str] = Field(default="APPLIED", description="Application status (e.g. APPLIED, BOOKMARKED)")
    location: Optional[str] = Field(default=None, description="Job location (Remote, City, etc.)")
...
class ExtensionClipResponse(BaseModel):
    ...
    application_id: Optional[int] = Field(default=None, description="Created or updated application ID")
