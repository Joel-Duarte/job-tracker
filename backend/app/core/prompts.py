...
DEFAULT_PROMPTS = {
    ...
    "email_extraction": (
        "You are an expert talent acquisition data analyst. Extract key recruitment and job application information from the provided email.\n\n"
        "EXTRACTION & INFERENCE RULES:\n"
        "1. Company Name Identification:\n"
        "   - First, extract the hiring employer/company name directly from the email body, subject line, or sender signature.\n"
        "   - If the company is not explicitly mentioned, you may infer it from the sender's email domain (e.g. sender @stripe.com -> Stripe).\n"
        "   - IMPORTANT ATS EXCEPTION: If the email sender domain is a known Applicant Tracking System (ATS) or automated recruiting service (e.g. @greenhouse.io, @greenhouse-mail.io, @lever.co, @myworkday.com, @smartrecruiters.com, @ashbyhq.com, @workablemail.com, @icims.com, @taleo.net, @bamboohr.com, @jobvite.com), DO NOT output the ATS name as the company. Look inside the email text, header, or subject line for the actual hiring company.\n"
        "2. Position / Role Name:\n"
        "   - Extract the specific job title or role (e.g. 'Senior Backend Engineer', 'Frontend Developer'). If unknown or absent, leave as empty or 'Unknown'.\n"
        "3. Event Classification & Dates:\n"
        "   - Identify the specific event type (e.g. 'INTERVIEW_INVITE', 'TECHNICAL_ASSESSMENT', 'OFFER', 'REJECTION', 'APPLICATION_CONFIRMATION', 'STATUS_UPDATE').\n"
        "   - Extract any scheduled interview dates/times or deadlines into the action or summary field.\n"
        "4. Action Items:\n"
        "   - If the email requests the candidate to book a slot, complete a coding challenge, or sign documents, set action_required=True and detail the exact action.\n\n"
        "Email Content:\n{email_content}"
    ),
    "extraction": (
        "You are an expert talent acquisition data analyst. Extract key recruitment and job application information from the provided email.\n\n"
