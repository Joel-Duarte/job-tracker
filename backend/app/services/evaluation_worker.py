...
async def process_evaluation_task(task_id: int) -> None:
    """
    ...
    async with concurrency_manager.acquire(provider_id, max_concurrency):
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                task.stage = "COMPLETE"
                task.result_json = assessment.model_dump()
                task.title_hint = f"{assessment.company} - {assessment.position}"
                task.completed_at = datetime.now(timezone.utc)
                await db.commit()
                logger.info("Intake evaluation task %d completed successfully for '%s'", task_id, task.title_hint)

            except Exception as err:
                logger.error("Failed processing intake task %d: %s", task_id, err, exc_info=True)
                task.status = "FAILED"
                task.stage = "FAILED"
