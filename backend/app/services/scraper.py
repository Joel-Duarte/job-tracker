...
(() => {
    try {
        ...
        const dialogSelectors = [
            ...
            '.modal-backdrop', '.cc-banner', '#cookie-notice', '.cookie-notice'
        ];
        ...
        const clutter = document.querySelectorAll('script, style, noscript, svg, nav, footer, header, iframe, canvas');
        clutter.forEach(el => {
            try { el.remove(); } catch(e) {}
        });

        // 4. Extract primary content if present, otherwise body text
        const mainEl = document.querySelector('main, [role="main"], article, #job-description, .job-description, .posting-content, .job-details');
...
async def _scrape_via_camofox(url: str, timeout_seconds: float = 25.0) -> Optional[ScrapedJobContent]:
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout_seconds) as client:
            ...
            if open_resp.status_code != 200:
                logger.warning("Camofox /tabs/open failed (%d): %s", open_resp.status_code, open_resp.text)
                return None
            ...
            # 3. Evaluate extraction & button clicking script inside page
            eval_resp = await client.post(
                f"{base_url}/tabs/{tab_id}/evaluate",
                json={"userId": user_id, "expression": EXPAND_AND_EXTRACT_JS},
            )
            ...
            if eval_resp.status_code == 200:
                ...
                if isinstance(raw_eval_result, str) and raw_eval_result.startswith("{"):
                    ...
                    try:
                        ...
                        if page_text:
                            logger.info("Successfully scraped %s via Camofox (%d chars)", url, len(page_text))
                        ...
                        logger.warning("Failed parsing Camofox JS result: %s", parse_err)

    except Exception as err:
        logger.warning("Camofox scraping error for %s: %s", url, err)
    finally:
        # 4. Always close the tab to prevent leaks
        if tab_id:
            try:
                async with httpx.AsyncClient(timeout=5.0) as client:
                    await client.delete(f"{base_url}/tabs/{tab_id}?userId={user_id}")
            except Exception as close_err:
                logger.debug("Non-fatal error closing Camofox tab %s: %s", tab_id, close_err)

    return None


async def _scrape_via_http_fallback(url: str, timeout_seconds: float = 15.0) -> ScrapedJobContent:
    ...
    async with httpx.AsyncClient(timeout=timeout_seconds, follow_redirects=True) as client:
        ...
        main_el = soup.find(["main", "article", "div"], class_=re.compile(r"job|posting|description", re.I))
        raw_text = main_el.get_text(separator="\n") if main_el else soup.get_text(separator="\n")
        cleaned_text = clean_extracted_text(raw_text)

        logger.info("Successfully scraped %s via HTTP fallback (%d chars)", url, len(cleaned_text))
        return ScrapedJobContent(
            title=title or "",
            text=cleaned_text,
            source_url=url,
...
async def scrape_job_url(url: str, timeout_seconds: float = 25.0) -> ScrapedJobContent:
    ...
    camofox_result = await _scrape_via_camofox(cleaned_url, timeout_seconds=timeout_seconds)
    ...
    try:
        return await _scrape_via_http_fallback(cleaned_url, timeout_seconds=min(timeout_seconds, 15.0))
    except Exception as fallback_err:
        logger.error("Both Camofox and HTTP fallback failed for %s: %s", cleaned_url, fallback_err)
