...
class ApplicationService:
    """Service class encapsulating business logic, queries, and database transactions for applications."""
    ...
    async def list_applications(
        ...
        for app in applications:
            ...
            has_pending_tasks = any(
                a.status == "PENDING" for a in (app.action_items or [])
            )
            ...
            if latest_stage != "Task Completed / Awaiting Response":
                for evt in app.events or []:
                    if evt.raw_payload and isinstance(evt.raw_payload, dict):
                        ...
                        if sched_val:
                            try:
                                scheduled_interview = datetime.fromisoformat(
                                    str(sched_val)
                                )
                                break
                            except Exception:
    ...
    async def get_applications_by_status(
        db: AsyncSession,
        status: AllowedApplicationStatus,
        limit: int = 20,
    ) -> list[ApplicationByStatusResult]:
        stmt = (
            select(
                ...
                func.count(func.distinct(ApplicationEventModel.id)).label(
                    "event_count"
                ),
    ...
    async def update_application(
        ...
        return await ApplicationService.get_application(db, app.id)
    ...
    async def transition_application(
        ...
        is_task_completed = (
            payload.interview_stage == "Task Completed / Awaiting Response"
        )
        ...
        return await ApplicationService.get_application(db, app_refreshed.id)
