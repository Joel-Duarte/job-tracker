...
class EmailAccountModel(Base):
    ...
    )  # e.g., "Personal Gmail", "Work Outlook"
    auth_type: Mapped[str] = mapped_column(
        String(50), default="IMAP", nullable=False
    )  # IMAP, GMAIL_OAUTH, MS_GRAPH_OAUTH
    ...
    sync_interval: Mapped[str | None] = mapped_column(
        String(50), default="1h", nullable=True
    )  # MANUAL, 15m, 1h, 6h, 24h, WEEKLY
    sync_schedule_time: Mapped[str | None] = mapped_column(
        String(20), default="09:00", nullable=True
    )  # "09:00" (24h)
    sync_schedule_day: Mapped[str | None] = mapped_column(
        String(20), default="MON", nullable=True
    )  # MON, TUE, etc.
