...
## Corpus Check
- 222 files · ~264,205 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2855 nodes · 5704 edges · 183 communities (124 shown, 59 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 668 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `dd513a41`
...
Cohesion: 0.14
Nodes (29): ApplicationModel, execute_analyze_pipeline_metrics(), execute_detect_stalled_applications(), execute_evaluate_ai_fit_score(), execute_get_application_details(), execute_list_applications(), execute_manage_action_items(), execute_manage_intake_queue() (+21 more)
...
Cohesion: 0.15
Nodes (38): AsyncSession, bulk_transition_applications(), clear_app_interview_guide(), generate_app_cover_letter(), generate_app_interview_guide(), generate_app_interview_guide_stream(), get_application(), get_applications_by_status() (+30 more)
...
Cohesion: 0.15
Nodes (22): AgentChatModel, ActionItemModel, ApplicationEmbeddingModel, ApplicationEventModel, Base, JobPostingModel, OtherEventModel, TraceEventModel (+14 more)
...
Cohesion: 0.13
Nodes (40): create_ai_provider(), _fetch_models_from_endpoint(), get_ai_health_endpoint(), get_global_settings(), _is_embedding_model(), _is_reasoning_model(), list_ai_providers(), list_ai_task_bindings() (+32 more)

### Community 16 - "routers/diagnostics.py"
Cohesion: 0.23
...
### Community 17 - "CandidateCVModel"
Cohesion: 0.10
Nodes (29): CandidateCVModel, delete_cv_profile(), enqueue_cv_profile_processing(), get_active_cv_profile(), get_cv_task_status(), parse_cv_document_file(), AsyncSession, BackgroundTasks (+21 more)
...
Cohesion: 0.08
Nodes (37): cancel_running_task(), get_running_task_ids(), PrioritySemaphore, ProviderConcurrencyManager, A semaphore that grants locks based on priority. Waiters with a lower priority…, Registers an in-memory running asyncio Task by database task ID., Removes a finished or cancelled task from the in-memory registry., Cancels an active background asyncio.Task in memory. Disconnects the active… (+29 more)
...
Cohesion: 0.18
Nodes (21): generate_query_embedding(), Generates a vector embedding for an incoming search query string using…, _clean_base_url(), get_active_llm_config_dict(), _get_cached_embeddings_model(), get_chat_model(), get_embeddings_model(), get_task_chat_model() (+13 more)

### Community 29 - "routers/intake.py"
Cohesion: 0.07
Nodes (58): AssessJobRequest, assess_job_lead(), bulk_delete_evaluation_tasks(), bulk_retry_evaluation_tasks(), cancel_evaluation_task(), clear_completed_evaluations(), confirm_job_assessment(), enqueue_job_assessment() (+50 more)
...
Cohesion: 0.10
Nodes (34): ProcessedEmailModel, Single source of truth for all email deduplication. Every email that passes…, SyncFolderRequest, EmailPayload, process_email_batch_sequential(), process_single_email_graph(), AsyncSession, Executes the LangGraph StateGraph pipeline for a single email payload. (+26 more)

### Community 32 - "AIProviderModel"
Cohesion: 0.24
...
Cohesion: 0.16
Nodes (16): AgentAPI, AIConfigAPI, EventsAPI, IntakeAPI, PromptsAPI, StagingAPI, SystemSettingsAPI, uiStore (+8 more)
...
Cohesion: 0.11
Nodes (44): CompanyModel, StagingItemModel, JobTrackerState, TypedDict, ExtractedEmailInfo, Structured extraction format returned by the LLM service., model_validator, Payload for user manual resolution/override of a staged email or job lead. (+36 more)
...
### Community 46 - "get_db"
Cohesion: 0.17
Nodes (13): get_db(), AsyncSession, asyncio, test_action_items_crud_and_filtering(), asyncio, AsyncSession, test_application_patch_updates(), test_application_transitions_and_deletion() (+5 more)

### Community 47 - "services/agent_tools.py"
Cohesion: 0.35
...
Cohesion: 0.11
Nodes (21): _encrypt_table_secrets(), Connection, upgrade(), decrypt_secret(), encrypt_secret(), _get_fernet(), mask_secret(), Encrypt a sensitive value, preserving already encrypted values. (+13 more)

### Community 54 - "test_llm_factory.py"
Cohesion: 0.17
Nodes (26): AsyncSession, Seeds missing prompts into DB upon boot without overwriting existing user…, seed_default_prompts(), ApplicationSummaryResult, EmailExtractionResult, HardMatches, ImpactReframingItem, LanguageMatchResult (+18 more)
...
Cohesion: 0.08
Nodes (34): delete_application(), Permanently deletes an application and its associated events, postings, and…, delete_evaluation_task(), Cancels an ongoing evaluation or dismisses a completed/failed evaluation task., bulk_dismiss_staging_items(), clear_resolved_staging_items(), get_staging_item(), list_staging_items() (+26 more)
...
### Community 67 - "assess_job_posting"
Cohesion: 0.17
Nodes (14): assess_job_posting(), JobAssessmentResult, Evaluates a job posting / JD against candidate CV for pre-application…, compute_programmatic_skill_match(), _is_skill_matched(), _normalize_token(), Checks if a JD required skill matches any skill in the candidate's CV profile., Computes Job Requirement Coverage Ratio between candidate CV skills and the… (+6 more)
...
### Community 82 - "_execute_evaluation_steps"
Cohesion: 0.53
...
### Community 101 - "generate_interview_guide_stream"
Cohesion: 0.24
Nodes (9): clear_interview_guide(), generate_interview_guide(), generate_interview_guide_stream(), AsyncSession, Async generator that executes `interview_guide_graph.astream(...)` and yields…, Coordinates candidate profile retrieval, job posting lookup, LangGraph…, Clears the existing interview guide for an application., Unit test for generate_interview_guide_stream async generator. (+1 more)
...
Cohesion: 0.07
Nodes (58): Any, normalize_job_url(), Cleans leading and trailing whitespace while preserving the exact original URL,…, cover_letter_node(), db_commit_node(), extraction_node(), fuzzy_match_node(), _get_db() (+50 more)
...
Cohesion: 0.19
Nodes (12): check_db_connection(), ensure_db_schema(), Tests the connection to PostgreSQL and logs the connected database name., Ensures required extensions exist, provisions any missing database tables from…, health_check(), lifespan(), get, Response (+4 more)

### Community 146 - "get_prompt_template"
Cohesion: 0.17
Nodes (17): ApplicationSummaryResult, get_prompt_template(), Retrieves prompt template from DB with in-memory caching, falling back to…, anonymize_and_parse_cv(), extract_job_spec(), generate_embedding(), get_active_llm_config(), Any (+9 more)

### Community 148 - "routers/llm.py"
Cohesion: 0.25
Nodes (14): LLMConfigModel, get_current_llm_config(), LLMConfigRead, LLMConfigUpdate, Any, AsyncSession, BaseModel, delete (+6 more)

### Community 149 - "PostgresTracer"
Cohesion: 0.27
...
### Community 168 - "test_bulk_transition.py"
Cohesion: 0.42
...
### Community 179 - "IntakeQueueDrawer.vue"
Cohesion: 0.22
Nodes (5): activeCount, hasItems, queue, STAGES, uiStore

### Community 180 - "clear_prompt_cache"
Cohesion: 0.25
Nodes (7): clear_prompt_cache(), Invalidates the in-memory prompt cache for a specific prompt or all prompts., asyncio, Unit test for in-memory prompt template cache and invalidation without DB., Unit test for Embeddings model instance caching and invalidation., test_embeddings_caching_unit(), test_prompt_caching_unit()

### Community 181 - "schemas/events.py"
Cohesion: 0.60
...
- **Why does `useUIStore` connect `endpoints.js` to `SettingsView.vue`, `ApplicationDetailDrawer.vue`, `OnboardingWizardModal.vue`, `ApplicationsView.vue`, `AgentChatView.vue`, `QueueView.vue`, `StagingView.vue`, `AssessmentsView.vue`, `AnalyticsView.vue`, `CandidateProfileView.vue`, `JobIntakeView.vue`, `EmailAccountsSettings.vue`, `FloatingQueueWidget.vue`, `ActionItemsView.vue`, `CoverLetterModal.vue`, `AppNavbar.vue`, `IngestModal.vue`, `MatchAnalysisModal.vue`, `LogActivityModal.vue`, `JobIntakeModal.vue`, `IntakeQueueDrawer.vue`, `SearchView.vue`, `index.js`, `InterviewReaderModal.vue`, `demoStorage.js`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `ApplicationModel` connect `ApplicationModel` to `admin.py`, `routers/extension.py`, `InterviewSimulatorService`, `generate_interview_guide_stream`, `CompanyModel`, `test_bulk_transition.py`, `load_settings`, `ApplicationEventModel`, `routers/search.py`, `normalize_job_url`, `services/agent_tools.py`, `get_db`, `asyncio`, `routers/action_items.py`, `archive_stale_applications`, `IntakeEvaluationTaskModel`, `routers/intake.py`, `EmailPayload`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `PostgresTracer` connect `PostgresTracer` to `InterviewSimulatorService`, `assess_job_posting`, `generate_interview_guide_stream`, `ApplicationEventModel`, `normalize_job_url`, `routers/ai_config.py`, `get_prompt_template`, `_execute_evaluation_steps`, `routers/llm.py`, `test_llm_factory.py`, `routers/agent_chat.py`, `EmailPayload`?**
