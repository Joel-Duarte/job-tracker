...
## Corpus Check
- 224 files · ~277,057 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2951 nodes · 5858 edges · 191 communities (121 shown, 70 thin omitted)
- Extraction: 88% EXTRACTED · 12% INFERRED · 0% AMBIGUOUS · INFERRED: 709 edges (avg confidence: 0.9)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `01616e04`
...
Cohesion: 0.07
Nodes (58): AgentChatRead, AgentChatRequest, AgentChatResponse, chat_with_agent(), ChatMessage, delete_chat(), get_chat(), list_chats() (+50 more)
...
Cohesion: 0.13
Nodes (20): Partially updates a job application and enqueues background vector embedding…, update_application(), AllowedApplicationStatus, ApplicationTransitionRequest, ApplicationUpdate, BulkTransitionRequest, BulkTransitionResult, Any (+12 more)
...
Cohesion: 0.12
Nodes (30): AgentChatModel, ActionItemModel, ApplicationEmbeddingModel, ApplicationEventModel, Base, JobPostingModel, OtherEventModel, TraceEventModel (+22 more)
...
Cohesion: 0.27
Nodes (19): get_application(), list_applications(), _to_utc(), ActionItemDetail, ApplicationAnalyzeSpecRequest, ApplicationByStatusResult, ApplicationDetailResponse, ApplicationEventDetail (+11 more)

### Community 16 - "routers/ai_config.py"
Cohesion: 0.11
Nodes (49): clear_embeddings_cache(), Clears cached Embeddings model instances., create_ai_provider(), delete_ai_provider(), delete_ai_task_binding(), _fetch_models_from_endpoint(), get_ai_health_endpoint(), get_global_settings() (+41 more)
...
Cohesion: 0.18
Nodes (21): _clean_base_url(), _get_cached_embeddings_model(), get_chat_model(), get_embeddings_model(), get_task_chat_model(), get_task_embeddings_model(), AsyncSession, Strips <think>...</think> reasoning tags from LLM output text. (+13 more)

### Community 29 - "routers/intake.py"
Cohesion: 0.07
Nodes (55): AssessJobRequest, assess_job_lead(), bulk_delete_evaluation_tasks(), bulk_retry_evaluation_tasks(), cancel_evaluation_task(), clear_completed_evaluations(), confirm_job_assessment(), delete_evaluation_task() (+47 more)
...
Cohesion: 0.14
Nodes (19): AgentAPI, AIConfigAPI, CandidateProfileAPI, EventsAPI, IntakeAPI, PromptsAPI, StagingAPI, SystemSettingsAPI (+11 more)
...
Cohesion: 0.06
Nodes (43): check_db_connection(), ensure_db_schema(), get_db(), AsyncSession, Tests the connection to PostgreSQL and logs the connected database name., Ensures required extensions exist, provisions any missing database tables from…, generate_query_embedding(), Generates a vector embedding for an incoming search query string using… (+35 more)
...
Cohesion: 0.08
Nodes (25): activeTab, copiedJd, copiedUrl, dismissedLinkedInUrl, dismissLinkedInWarning(), executeEnqueue(), extractUrls(), handleBulkPromptDecision() (+17 more)

### Community 46 - "extract_usage_from_payload"
Cohesion: 0.13
Nodes (23): update_pricing_rates_endpoint(), calculate_comparative_provider_costs(), calculate_cost_and_savings(), extract_usage_from_payload(), get_all_pricing_rates(), _match_pricing_key(), Matches a model name string (including path / provider prefixes) to a pricing…, Returns all current pricing rates as a list with custom overrides applied. (+15 more)

### Community 47 - "AIProviderModel"
Cohesion: 0.33
Nodes (14): AIProviderModel, AITaskBindingModel, check_ai_provider_health(), asyncio, AsyncSession, fixture, reset_health_cache(), test_ai_health_cache_ttl_and_invalidation() (+6 more)

### Community 48 - "EmailAccountModel"
Cohesion: 0.11
Nodes (27): EmailAccountModel, _clean_header(), fetch_emails_from_account(), _fetch_imap_emails_sync(), datetime, Fetches emails using either modern OAuth adapters (Google Workspace, Microsoft…, Helper to decode encoded email headers (e.g. Subject)., Synchronous worker that performs actual IMAP connection and retrieval using… (+19 more)
...
Cohesion: 0.21
Nodes (17): analyze_app_job_spec(), bulk_transition_applications(), generate_app_cover_letter(), generate_app_interview_guide_stream(), get_applications_by_status(), get_cover_letter(), AsyncSession, BackgroundTasks (+9 more)
...
### Community 67 - "clear_prompt_cache"
Cohesion: 0.13
...
Cohesion: 0.22
Nodes (7): _get_or_generate_secret_key(), model_validator, Always constructs the connection URI dynamically from current settings., Reads persistent SECRET_KEY from PROJECT_ROOT/data/.sec_key or auto-generates a…, Settings, test_secret_key_validation_in_non_dev_environments(), BaseSettings
...
### Community 97 - "clear_app_interview_guide"
Cohesion: 0.50
Nodes (4): clear_app_interview_guide(), delete_application(), delete, Permanently deletes an application and its associated events, postings, and…
...
### Community 110 - "CandidateCVModel"
Cohesion: 0.05
Nodes (83): Any, get_setting(), get_system_settings_model(), load_settings(), Any, AsyncSession, Saves system settings from a dictionary supporting lower-case and upper-case…, Fetches the singleton system settings model (id=1), creating it if it does not… (+75 more)
...
Cohesion: 0.11
Nodes (41): asyncio, ApplicationModel, CompanyModel, archive_stale_applications(), Any, AsyncSession, Finds all applications in active stages where last_activity_at (or…, test_get_analytics_overview_unit() (+33 more)

### Community 146 - "test_new_features.py"
Cohesion: 0.10
Nodes (27): prune_and_sanitize_tool_output(), Any, Sanitizes and prunes tool execution output payloads: - Parses string payloads…, generate_app_interview_guide(), GenerateInterviewGuideRequest, clear_interview_guide(), generate_interview_guide(), generate_interview_guide_stream() (+19 more)
...
Cohesion: 0.19
Nodes (12): clean_html_text(), Converts HTML-rich or markup-tainted text into clean, readable plain text: 1.…, intake_pasted_text(), intake_uploaded_files(), Ingests raw pasted email text, thread, or job communication by queuing an AI…, Ingests drag-and-drop uploaded email files (.eml, .msg, .txt) by queuing an AI…, test_clean_html_text_decodes_entities_and_formats_breaks(), test_clean_html_text_none_and_empty() (+4 more)
...
### Community 190 - "test_persist_job_assessment_unrestricted_urls"
Cohesion: 0.50
...
- **Why does `useUIStore` connect `endpoints.js` to `SettingsView.vue`, `ApplicationDetailDrawer.vue`, `OnboardingWizardModal.vue`, `ApplicationsView.vue`, `AgentChatView.vue`, `QueueView.vue`, `StagingView.vue`, `AssessmentsView.vue`, `AnalyticsView.vue`, `CandidateProfileView.vue`, `JobIntakeView.vue`, `EmailAccountsSettings.vue`, `FloatingQueueWidget.vue`, `ActionItemsView.vue`, `CoverLetterModal.vue`, `AppNavbar.vue`, `IngestModal.vue`, `MatchAnalysisModal.vue`, `JobIntakeModal.vue`, `IntakeQueueDrawer.vue`, `LogActivityModal.vue`, `InterviewReaderModal.vue`, `demoStorage.js`, `SearchView.vue`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `ApplicationModel` connect `ApplicationModel` to `process_email_batch_sequential`, `PostgresTracer`, `persist_or_stage_job_assessment`, `services/agent_tools.py`, `process_evaluation_task`, `EmailPayload`, `main.py`, `ExtractedEmailInfo`, `ApplicationEventModel`, `CandidateCVModel`, `routers/extension.py`, `test_new_features.py`, `routers/action_items.py`, `test_extension.py`, `routers/intake.py`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `get_db()` connect `main.py` to `PostgresTracer`, `conftest.py`, `services/agent_tools.py`, `schemas/analytics.py`, `process_evaluation_task`, `EmailPayload`, `ApplicationModel`, `ApplicationEventModel`, `CandidateCVModel`, `AIProviderModel`, `routers/ai_config.py`, `routers/extension.py`, `test_email_accounts.py`, `routers/action_items.py`, `routers/email_accounts.py`, `test_extension.py`, `routers/llm.py`?**
