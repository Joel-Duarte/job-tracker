...
## Corpus Check
- 259 files · ~326,279 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3513 nodes · 6747 edges · 228 communities (146 shown, 82 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 755 edges (avg confidence: 0.89)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f8325705`
...
## Communities (228 total, 82 thin omitted)
...
Cohesion: 0.03
Nodes (53): SystemSettingsAPI, availableMailFolders, availableModels, copiedRedirectUri, createdEmailAccountId, currentEmailProvider, currentStep, customModelMode (+45 more)
...
### Community 8 - "main.py"
Cohesion: 0.09
Nodes (25): lifespan(), delete_application(), get_staleness_stats(), AsyncSession, delete, get, post, # TODO: Trigger re-indexing of the embedding for this application based on the… (+17 more)
...
Cohesion: 0.04
Nodes (41): activeCancelTask, activeCount, activeFixJDTask, bulkDeleteSelected(), bulkRetrySelected(), clearSelection(), completedCount, deleteTask() (+33 more)
...
Cohesion: 0.05
Nodes (78): normalize_job_url(), Cleans leading and trailing whitespace while preserving the exact original URL,…, cover_letter_node(), db_commit_node(), extraction_node(), fuzzy_match_node(), _get_db(), is_email_already_processed() (+70 more)

### Community 15 - "get_prompt_template"
Cohesion: 0.13
Nodes (23): clear_prompt_cache(), get_prompt_template(), AsyncSession, Invalidates the in-memory prompt cache for a specific prompt or all prompts., Retrieves prompt template from DB with in-memory caching, falling back to…, PromptModel, get_prompt(), list_prompts() (+15 more)
...
Cohesion: 0.14
Nodes (22): CVAnonymizationResult, DomainExperienceItem, assess_job_posting(), calibrate_assessment_score_and_recommendation(), Applies mathematical bounding and recommendation synchronization to eliminate…, Evaluates a job posting / JD against candidate CV for pre-application…, compute_programmatic_skill_match(), _is_skill_matched() (+14 more)
...
Cohesion: 0.10
Nodes (34): ApplicationEventModel, OtherEventModel, delete_event(), EmailPayload, model_validator, Payload for user manual resolution/override of a staged email or job lead., StagingItemResolve, process_email_batch_sequential() (+26 more)
...
### Community 54 - "patch"
Cohesion: 0.25
Nodes (14): Seeds missing prompts into DB upon boot without overwriting existing user…, seed_default_prompts(), extract_email_info(), Extracts structured job application metadata from email body using LangChain…, asyncio, AsyncSession, test_extract_email_info_runnable(), test_extract_job_spec_runnable() (+6 more)

### Community 55 - "_execute_evaluation_steps"
Cohesion: 0.10
Nodes (38): get_setting(), Retrieves a specific system setting by key asynchronously., _execute_application_qa_steps(), _execute_company_research_steps(), _execute_cover_letter_steps(), _execute_cv_extraction_steps(), _execute_email_sync_steps(), _execute_evaluation_steps() (+30 more)
...
Cohesion: 0.10
Nodes (27): apiClient, AgentAPI, AIConfigAPI, uiStore, uiStore, openCoverLetterModal(), router, uiStore (+19 more)
...
### Community 67 - "routers/staging.py"
Cohesion: 0.10
Nodes (23): ApplicationEmbeddingModel, clip_job_pre_extracted(), clip_job_url(), _extract_text_from_html(), AsyncSession, post, Directly accepts pre-extracted DOM metadata (company, title, description, url)…, Strips HTML tags, removes scripts/styles/nav/forms, and cleans visible text… (+15 more)
...
Cohesion: 0.09
Nodes (46): AIHealthStatusRead, AIProviderCreate, AIProviderModel, AIProviderModelsResponse, AIProviderRead, AIProviderTestResponse, AIProviderUpdate, AITaskBindingCreate (+38 more)

### Community 70 - "routers/system_settings.py"
Cohesion: 0.18
Nodes (18): update_global_settings(), get_system_settings(), AsyncSession, get, patch, post, Validates connectivity to a search provider (such as SearXNG) before saving…, test_search_provider() (+10 more)
...
Cohesion: 0.16
Nodes (15): ActionItemsAPI, AnalyticsAPI, EventsAPI, IntakeAPI, PromptsAPI, StagingAPI, SystemAPI, DEFAULT_ALIGNMENT (+7 more)
...
### Community 81 - "get_db"
Cohesion: 0.25
Nodes (9): get_db(), AsyncSession, asyncio, test_action_items_crud_and_filtering(), asyncio, AsyncSession, test_extension_clip_job_direct(), test_extension_clip_url_pipeline() (+1 more)
...
Cohesion: 0.08
Nodes (47): asyncio, CompanyModel, build_company_research_query(), Constructs a multi-factor DuckDuckGo search query anchored with domain and…, test_get_analytics_overview_unit(), test_get_funnel_performance_metrics_unit(), test_get_role_alignment_unit(), test_pipeline_funnel_active_and_dropped_unit() (+39 more)
...
### Community 91 - "FailoverChatModel"
Cohesion: 0.29
...
Cohesion: 0.14
Nodes (20): JobPostingModel, CandidateCVModel, build_dossier(), build_structured_spec(), is_database_empty(), maybe_seed_dev_data(), AsyncSession, Checks if development seeding is enabled and database is empty. If both… (+12 more)
...
### Community 161 - "resolve_company_about_url"
Cohesion: 0.25
...
### Community 183 - "test_application_transitions.py"
Cohesion: 0.57
...
Cohesion: 0.17
Nodes (20): AgentChatModel, ApplicationModel, archive_stale_applications(), delete_stale_agent_chats(), Any, AsyncSession, Finds all applications in active stages where last_activity_at (or…, Background worker that runs once every interval_seconds. (+12 more)

### Community 232 - "routers/search.py"
Cohesion: 0.31
...
## Knowledge Gaps
- **826 isolated node(s):** `uiStore`, `appStore`, `{ isCompanyDrawerOpen, selectedCompanyId }`, `company`, `isLoading` (+821 more)
...
- **Why does `useUIStore` connect `uiStore.js` to `SettingsView.vue`, `ApplicationDetailDrawer.vue`, `OnboardingWizardModal.vue`, `ApplicationQuestionModal.vue`, `ApplicationsView.vue`, `AgentChatView.vue`, `IntakeQueueDrawer.vue`, `QueueView.vue`, `StagingView.vue`, `CompanyDetailDrawer.vue`, `AssessmentsView.vue`, `AnalyticsView.vue`, `CandidateProfileView.vue`, `JobIntakeView.vue`, `EmailAccountsSettings.vue`, `FloatingQueueWidget.vue`, `ActionItemsView.vue`, `CoverLetterModal.vue`, `LogActivityModal.vue`, `AppNavbar.vue`, `IngestModal.vue`, `MatchAnalysisModal.vue`, `JobIntakeModal.vue`, `SearchView.vue`, `CompaniesView.vue`, `InterviewReaderModal.vue`, `endpoints.js`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `ApplicationModel` connect `ApplicationModel` to `PostgresTracer`, `services/agent_tools.py`, `main.py`, `persist_or_stage_job_assessment`, `normalize_job_url`, `datetime`, `ActionItemModel`, `load_settings`, `test_companies_router.py`, `process_evaluation_task`, `ApplicationEventModel`, `ExtractedEmailInfo`, `test_ai_config.py`, `test_application_transitions.py`, `test_analytics.py`, `routers/staging.py`, `get_db`, `CompanyModel`, `routers/search.py`, `JobPostingModel`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `CompanyModel` connect `CompanyModel` to `test_analytics.py`, `PostgresTracer`, `routers/staging.py`, `services/agent_tools.py`, `process_evaluation_task`, `ApplicationModel`, `routers/search.py`, `ApplicationEventModel`, `normalize_job_url`, `JobPostingModel`, `ExtractedEmailInfo`, `get_db`, `datetime`, `test_ai_config.py`, `test_application_transitions.py`, `load_settings`, `test_companies_router.py`?**
...
- **What connects `uiStore`, `appStore`, `{ isCompanyDrawerOpen, selectedCompanyId }` to the rest of the system?**
